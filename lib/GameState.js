 ...
 * @param {Array|GameState} mDeckOrGameState If a list (array) of cards, a GameState of a new game is created;
 * 		if a GameState object, its data is cloned into this new GameState object.
 * @constructor
 */
function GameState (mDeckOrGameState)
{
	this.oEvents = $({});

	// If "new" missing, fix it.
	if (! (this instanceof GameState)) {
		return new GameState(mDeckOrGameState);
	}

	if (! (mDeckOrGameState instanceof GameState)) {
		const aDeck = mDeckOrGameState;
		...
		const oOriginal = mDeckOrGameState;

		for (const sProperty of [
			'aGameLog', 'aDeck', 'sGameMode', 'sGameModeBy', 'oPot', 'oDiscardedCards', 'oHands', 'oPlayedCards', 'aLastTrick', 'oStats'
		]) {
			this[sProperty] = _.cloneDeep(oOriginal[sProperty]);
		}
	}
}
...
GameState.GetTrickTaker = function GameState_GetTrickTaker (aTrick)
{
	...
	const aSortedEligibleTakerCards = aTrick
		...
		{
			return oAction.card.GetSuit() == sRequestedSuit || Zole.IsZoleTrumpCard(oAction.card);
		})
		// And then we can apply the global sorting, out of the context of the trick / suits.
		.sort(function (oActionA, oActionB)
		{
			return Zole.CardToStrengthIndex(oActionA.card) - Zole.CardToStrengthIndex(oActionB.card);
		});
 ...
 * @return {CardSet}
 */
GameState.SelectCards = function GameState_SelectCards (oHand, fFilter)
{
...
GameState.GetAllowedResponses = function GameState_GetAllowedResponses (oLeadingCard, oHand)
{
	...
	if (Zole.IsZoleTrumpCard(oLeadingCard)) {
		oAllowedCards = GameState.SelectCards(oHand, Zole.IsZoleTrumpCard);
	}
	else {
		let sRequestedSuit = oLeadingCard.GetSuit();
		oAllowedCards = GameState.SelectCards(oHand, function (oCard)
		{
			return oCard.GetSuit() == sRequestedSuit && ! Zole.IsZoleTrumpCard(oCard);
		});
	}
...
GameState.prototype.AssertCardIsValid = function AssertCardIsValid (oCard)
{
	...
	if (oCard.IsKnown() && ! Zole.IsValidZoleCard(oCard)) {
...
GameState.prototype.Clone = function GameState_Clone ()
{
 ...
 * @return {boolean}
 */
GameState.prototype.IsGameOver = function GameState_IsGameOver ()
{
	return (this.GetAllowedActions().size <= 0);
};
...
GameState.prototype.SetDeck = function GameState_SetDeck (aCards)
{
	...
	if (oUniqueCards.Size() != Zole.aZoleStrength.length) {
			...
			if ('player' in oEntry.data) {
				oFinalLogEntry = this.ChooseGameMode(
					...
					'cards' in oEntry.data
						? new CardSet(JsCard.ConstructJsCards(oEntry.data.cards))
						: undefined
				);
			}
			else {
...
GameState.prototype.ChooseGameMode = function ChooseGameMode (sPlayer, sMode, oRevealedPotCards)
{
	...
	if (sMode == 'augšā') {
		...
		if (this.oPot.AreAllKnown()) {
			this.oStats.pot.points += Zole.CountPoints(this.oPot);
		}
	}
...
GameState.prototype.DiscardCard = function GameState_DiscardCard (sPlayer, oCard)
{
	...
	if (oCard.IsKnown()) {
		this.oStats[sPlayer].points += Zole.CountPoints([oCard]);
	}
...
GameState.prototype.GetAllowedPlayCardActions = function GameState_GetAllowedPlayCardActions ()
{
	...
	if (! this.sGameMode) {
		// Keep sPlayer undefined, no player may play a card at this point.
	}
	// A new trick:
	else if (this.aLastTrick.length == 3) {
		sPlayer = GameState.GetTrickTaker(this.aLastTrick);

		if (this.sGameMode == 'mazā zole' && sPlayer == this.sGameModeBy) {
			// If the player of "mazā zole" has taken a trick, they're lost and the game is over.
			sPlayer = undefined;
		}
	}
	// Within a trick:
...
GameState.prototype.PlayCard = function GameState_PlayCard (sPlayer, oCard)
{
	...
	if (this.GetAllowedPlayCardActions().size <= 0) {
		throw new GameState.GameRulesException(
			["PlayCard: This is not the time to be playing cards!", this.sGameMode]
		);
	}
	// A new trick:
	...
	if (this.aLastTrick.length >= 3) {
		...
		this.oStats[sTrickTakenBy].points += Zole.CountPoints(
	...
	if ((this.sGameMode == 'augšā' || this.sGameMode == 'zole') && this.oHands[sPlayer].Size() === 0 && this.aLastTrick.length >= 3) {
		...
		const oAllCards = new CardSet(Zole.GetZoleDeck());
		...
		if (! oToBeRevealedCards.AreAllKnown()) {
			...
			if (this.sGameMode == 'augšā') {
				for (const oCard of this.oDiscardedCards) {
					this.oStats[this.sGameModeBy].points += Zole.CountPoints([oCard]);
				}
			}
			else {
				this.oStats.pot.points += Zole.CountPoints(this.oPot);
 ...
 * Calculates chips according to the game state, not considering pules or other factors.
 */
GameState.prototype.CalculateGameChips = function GameState_CalculateGameChips ()
{
	// The result as described by the chips.
	const oChips = {A: 0, B: 0, C: 0};
	const oStats = this.oStats;

	switch (this.sGameMode) {
	case 'augšā':
	case 'zole':
		const sLielais = this.sGameModeBy;
	...
	case 'galdiņš':
		...
		// Each of the loser gives other players (also, other losers) two chips to each.
	...
	case 'mazā zole':
		const sMazais = this.sGameModeBy;
	...
	default:
		console.error('Unexpected game mode: ', this.sGameMode);
	}
