...
class PlayerOrder
{
		...
		return new Proxy(this, {
			...
			get: function (oPlayerOrder, mProp)
			{
				...
				// Static properties of `PlayerOrder`.
					...
					if (typeof mProp == 'string' && ! (mProp in oPlayerOrder)) {
						console.error('Proxy for PlayerOrder: Unexpected property requested. ',
							mProp, ' is not a player identifier, neither a method.'
						);

						if (mProp == 'length') {
							// "Lodash" checks for `length` when cloning.
							console.error('Do not use a generic `_.clone*` on this object! See `Clone` method!');
						}
					}
	...
	/**
	 * @param {string} sPlayerLetter (in the current `GameState`).
	...
	/**
	 * @param {int|string} iPlayerIndex Player index (in `GameRun.aPlayers`).
	...
	Import (oPlayerOrderByLetter)
	{
		...
		{
			if (sLetter in oPlayerOrderByLetter) {
				iLetters++;
				aPlayerOrder.push(oPlayerOrderByLetter[sLetter]);
			}
			else {
				if (_.size(oPlayerOrderByLetter) != iLetters) {
