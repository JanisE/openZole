...
//	console.log('UiAdapter_CardsJs.AddGameChipsToResult(', JSON.stringify(oGameChips), ')');
	...
	return this.pIsSetUp.then(() =>
	{
		const jqGameChips = $('table.chips tbody tr:first').find('td:gt(0)');
...
//	console.log('UiAdapter_CardsJs.OnStateChange: ', JSON.stringify(oLogEntry));
	...
	return new Promise((fResolve) =>
	{
		...
		case 'mode':
			...
			$('table.chips tbody tr:first .game_mode').text(
				this.oGameState.sGameMode
				+ (
					oLogEntry.data.mode == 'augšā' || oLogEntry.data.mode == 'zole' || oLogEntry.data.mode == 'mazā zole'
					? ' (' + this.aPlayers[this.oPlayerOrder[oLogEntry.data.player]].GetInfo().name + ')' : ''
				)
			);
...
{
	// Sort after a time-out, so the dealing is done before the sorting.
	setTimeout(() => {
