...
function Dealer (oEventInfo)
{
	...
	this.oPlayerOrder = new PlayerOrder();
	...
	// Dummy version/Placeholder values. See `SetUiAdapter`.
...
Dealer.prototype.SetPlayerOrder = function Dealer_SetPlayerOrder (iPlayerA, iPlayerB, iPlayerC)
{
	...
	this.oPlayerOrder.SetOrder(iPlayerA, iPlayerB, iPlayerC);
};
...
//	console.log('OnGameOver');
	...
	const oGame = this.oGameRun.AddGame(
		this.oPlayerOrder,
		this.oGameState
	).Clone();

	this.oUiAdapter.OnGameOver(oGame)
	.then(() =>
	{
		if (this.oGameRun.GetGameCount() < this.iGamesToPlay) {
			this.oPlayerOrder.ShiftPlayers();
			this.StartGame();
		}
		else {
			console.log('Example output by `Dealer.OnGameOver`:');
			console.log('Totals:', JSON.stringify(this.oGameRun.GetTotals(), null, "\t"));
			console.log('Summary of all games:',JSON.stringify(this.oGameRun.GetSummaryOfAllGames()));
			console.log('Full export of "GameRun":',JSON.stringify(this.oGameRun.Export()));
